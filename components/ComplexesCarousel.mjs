...
import { COMPLEXES } from '../src/data/complexes.mjs';
      ...
      <div ref={ref} className="flex snap-x gap-4 overflow-x-auto pb-2">
        {COMPLEXES.map((item) => (
          <article key={item.id} className="card-lift min-w-[320px] snap-start rounded-3xl border border-[#e4d8c8] bg-[#fcfaf6] p-5">
            ...
            <ul className="mt-2 space-y-1 text-sm text-[#655949]">{item.tags.map((tag) => <li key={tag}>• {tag}</li>)}</ul>
            <a href={item.href} target="_blank" rel="noreferrer" className="mt-5 inline-block rounded-full bg-[#c6a46c] px-4 py-2 text-sm font-semibold text-[#2f2417]">Подробнее</a>
          </article>
        ))}
      </div>
    </div>
